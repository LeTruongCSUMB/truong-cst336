...
//displays hint at cost of a guess
...
$(".listBtn").on("click", function() {
    ...
    $("#prevWords").append("<br />");
    $("#prevWords").append("<span class='prevWords'> " + solved[i] + "</span>");
    }
    $(this).prop('disabled', true);
});
...
function endGame(win) {
    ...
    if (win) {
        $('#won').show();
        let solved = localSorage.getItem('prevWords') ? JSON.parse(localStorage.getItem('prevWords')) : [];
        localStorage.setItem('prevWords', JSON.stringify(solved));
        const data = JSON.parse(localStorage.getItem('prevWords'));
        solved.push(selectedWord);
        localStorage.setItem('prevWords', JSON.stringify(solved));
    } else {
        $('#lost').show();
    }
